...
c mean_S_c004_v1.f, USM, 2017-11-14
c   extract only S 
...
!      Sa(i,j,k) = Sa(i,j,k) + ww(i+(j-1)*idm)/float(numfls)
            ...
            if (k.eq.1.and.i.eq.1450.AND.j.eq.1864) then
                write(6,*) w(i+(j-1)*idm),w(i+(j-1)*idm)
                call flush(6) 
            end if
...
!     &             access='append')
     ...
     &             access='append')
         ...
         do k=1,kdm 
           ...
           write(100) Ta(1+blki*(iblk-1)-halo:blki*iblk+halo,    !99 => 100
...
!         close(99) 
      ...
      do k=1,kdm
        write(6,*) k,Ta(1450,1864,k)
!        write(6,*) k,Ta(1450,1864,k),Sa(1450,1864,k)
        call flush(6)
      enddo

      write(6,*) 'calculating mean S is finished'
      stop
      end
