...
!     &        'HarmonicFit/chf_modeV_'
     ...
     &     0.5* ( cmplx(blk_modeU(i,j,2,l), -1*blk_modeU(i,j,3,l))
     &           +cmplx(blk_modeU(i+1,j,2,l),-1*blk_modeU(i+1,j,3,l)))
     &       * cmplx( blk_modeP(i,j,2,l), blk_modeP(i,j,3,l) ) * z )
        cblk_fluxV(i,j,l) = 0.5* real(
     &     0.5*( cmplx(blk_modeV(i,j,2,l), -1*blk_modeV(i,j,3,l))
     &          +cmplx(blk_modeV(i,j+1,2,l),-1*blk_modeV(i,j+1,3,l)))
     &       * cmplx( blk_modeP(i,j,2,l), blk_modeP(i,j,3,l) ) * z )
             endif
