       ...
       !!! Check to make sure infile_depth is not reading infile_PLAT.
       !!! 12/09/17 GRS.
...
!     & '_'//numstr(iblk)//'_v918.BinF'
     ...
     & 'HarmonicFit/chf_modeP_'//runid//'_blk_'//numstr(jblk)//
     &  '_'//numstr(iblk)//'.BinF'
     ...
     &         (blkj*(jblk-1)+1):(blkj*jblk),1:MEIG ) =
     &          blk_mHFp(1:iblk,1:jblk,1:MEIG,3)
     ...
     &            status='OLD', access='sequential',action='READ')
            ...
            do j = 1,blkj
              ...
              zbot = depth(i,j)
            do l = 1,MEIG
              ! p_n = blk_mHFp(i,j,l,2)
              p_n = ( blk_mHFp(i,j,l,2)**2 + blk_mHFp(i,j,l,3)**2)**0.5
              eigval = eval(i,j,l)

              glb_APE( blki*(iblk-1)+i, blkj*(jblk-1)+j,l ) = 1/rho0 
     &           * p_n**2 * zbot/4 * eigval
            enddo !l
            enddo !j
            enddo !i
     ...
     & 'modes_global/glb_VM_HFit_P_v2.BinF'
     ...
     & 'modes_global/glb_APE_mode'//numstr(l)//'_v2.BinF'
