       ...
       ! Mode 5 looks too strong, so modify code to
       ! remove each vertical mode as I fit it.
       implicit none
       ...
       real, allocatable, dimension(:,:,:) :: glb_mHFp_mean
       ...
       !Pass in parameters.
       parameter(runid='061')
       parameter(idm=4500,jdm=3528)
       parameter(blki=150,blkj=196,mbdy=3)
       parameter(IL=1,IU=5)
       MEIG = IU-IL+1
     ...
     & 1:2,1:MEIG))
       ...
       allocate( glb_mHFp_mean(1:idm,1:jdm,1:MEIG) )
...
!     & '_'//numstr(iblk)//'_v918.BinF'
     ...
     &     'modes/P_vmhf_'//runid//'_blk_'//numstr(jblk)//'_'
     &     //numstr(iblk)//'_jan23.BinF'
!     &     'HarmonicFit/hfvmP_'//runid//'_blk_'//numstr(jblk)//
!     &     '_'//numstr(iblk)//'_v18.BinF'
           ...
           if (isThere) then        
             ...
             glb_mHFp_mean( (blki*(iblk-1)+1):(blki*iblk),
     ...
     &         (blkj*(jblk-1)+1):(blkj*jblk),1:MEIG ) =
     &          blk_mHFp(1:iblk,1:jblk,3,1:MEIG)

            ! Load Maarten's eigenvalues 
            ! From mbui/hycom/GLBc0.08/eigenval/NEIG_c008_v3.f 
            ! EVAL = k_n^2/(omm2^2-f^2)
            infileK = '/p/work1/mbui/hycom/GLBc0.08/expt_06.1/modes/'
     ...
     &          numstr(iblk)//'.BinF'
            ...
            infile_plat='/p/work1/mbui/hycom/GLBc0.08/expt_06.1/'
     &       //'griddata/plat_061_blk_'//numstr(jblk)//'_'//
     &         numstr(iblk)//'.BinF'
     ...
     &            status='OLD', access='sequential',action='READ')
            ...
            ! Load depth.
            infile_depth='/p/work1/mbui/hycom/GLBc0.08/expt_06.1/'
     ...
     &            status='OLD', access='sequential',action='READ')
            ...
            do l = 1,MEIG
            p_n = ( blk_mHFp(i,j,2,l)**2 + blk_mHFp(i,j,3,l)**2 )**0.5
              eigval = eval(i,j,l)
     ...
     &           * p_n**2 * zbot/4 * eigval
       ...
       outfileP='/p/work1/grstephe/hycom/GLBc0.08/expt_06.1/'//
     & 'modes_global/glb_VM_HFit_P_jan23.BinF'
     ...
     &   form='unformatted',access='sequential')
       ...
       do l=1,MEIG 
         outfileAPE='/p/work1/grstephe/hycom/GLBc0.08/expt_06.1/'//
     & 'modes_global/glb_APE_mode'//numstr(l)//'_jan23.BinF'
