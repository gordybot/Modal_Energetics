      ...
      ! Fixed allocation of Pbot_ to match new file size.
...
!     &       'HarmonicFit/hfvmP_'//runid//
     &       'modes/'//'Pbot_vmhf_'//runid//
     &       '_blk_'//numstr(jblk)//'_'//
     ...
     &                (0.5*(pscy(i,j+1)+pscy(i,j)))
            ...
            ! Change the third index from [2,3] to [1,2].
            ! Just kidding -- the barotropic velocities should
            ! actually be 2 / COS and 3 / SIN.
     ...
     &                 cmplx( blk_modeP(i,j,1,l), blk_modeP(i,j,2,l) )
