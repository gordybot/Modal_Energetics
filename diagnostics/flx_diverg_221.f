      ...
      real, allocatable, dimension(:,:,:) :: glb_flx
      real, allocatable, dimension(:,:) :: glb_flx_div
      real, allocatable, dimension(:,:) :: blk_pscx, blk_pscy
     ...
     &            '81','82','83','84','85','86','87','88','89','90'/
      ...
      allocate( glb_flx(1-mbdy:idm+mbdy, 1-mbdy:jdm+mbdy,1:2) )
     ...
     &              form='unformatted')
        read(17) glb_flx(1:idm,1:jdm,1)
        read(17) glb_flx(1:idm,1:jdm,2)
        close(17)

        ! Wraparound glb_flx
        glb_flx(1-mbdy:0,1:jdm,1:2) = glb_flx(idm+1-mbdy:idm,1:jdm,1:2)
        glb_flx(idm+1-mbdy:idm,1:jdm,1:2) = glb_flx(1:mbdy,1:jdm,1:2)
     ...
     &        ( glb_flx( offset_i+i+1, offset_j + j, 1)
     &        - glb_flx( offset_i+i-1, offset_j + j, 1) )
     &        / ( blk_pscx( i, j) + 
     &            0.5*( blk_pscx( i-1, j)
     &                 +blk_pscx( i+1, j) ) )
     &      + ( glb_flx( offset_i+i, offset_j + j+1, 2)
     &        - glb_flx( offset_i+i, offset_j + j-1, 2) )
     &        / ( blk_pscy( i, j) +
     &            0.5*( blk_pscy( i, j+1)
     &                 +blk_pscy( i, j-1) ) )
